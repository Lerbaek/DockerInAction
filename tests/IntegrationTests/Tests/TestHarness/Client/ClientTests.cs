...
public class ClientTests : IAsyncDisposable
{
    ...
    public async Task Get_RequestIsReceived_PaymentIsPublished()
    {
        ...
        // Act
        var response = await _httpClient.GetAsync("/PaymentGenerator", TestContext.Current.CancellationToken);

        // Assert
        response.Invoking(r => r.EnsureSuccessStatusCode()).Should().NotThrow();
        var consumed = await _testHarness.Published.Any<Payment>(TestContext.Current.CancellationToken);
